...
def evaluate_model(env_id: str, algo: str, eval_path: Path):
    eval_cfg = get_run_config()['Evaluation']
    ...
    venv = create_eval_envs(env_id, n_eval_envs=eval_cfg['n_eval_envs'], seed=eval_cfg['eval_seed'],
                            video_logging_path=eval_path, vec_normalize_path=eval_cfg['vec_normalize_load_path'])
    ...
    save_results(eval_path=eval_path, model_path=model_path, episode_rewards=episode_rewards,
...
def save_results(eval_path: Path, model_path: str, episode_rewards: float,
    ...
    with open(eval_path / eval_file_name, 'w') as f:
        ...
        f.write("-" * 50 + "\n")
        f.write(f"Evaluation seed: {get_run_config()['Evaluation']['eval_seed']}\n")
        f.write(f"Model path: {model_path}\n")
        f.write(f"Number of evaluation episodes: {get_run_config()['Evaluation']['n_eval_episodes']}\n")
        f.write("-" * 50 + "\n")
